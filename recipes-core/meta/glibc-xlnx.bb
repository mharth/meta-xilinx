...
FILES_${PN} += " /lib/${CSL_TARGET_SYS} /usr/lib/${CSL_TARGET_SYS}"
...
do_install() {
	...
	find $root -path "*/${CSL_TARGET_SYS}/*/ld*.so*" -exec cp -a {} ${D}/lib \;
	...
	if [ "${CSL_TARGET_SYS}" == "aarch64-linux-gnu" ] ||  \
	   [ "${CSL_TARGET_SYS}" == "arm-linux-gnueabihf" ]; then
		...
		file=$(basename $(readlink ${D}/lib/$linker_name))
		rm ${D}/lib/$linker_name
		ln -s ./$file ${D}/lib/$linker_name

		mv ${D}/usr/lib/${CSL_TARGET_SYS}/* ${D}/usr/lib
		rm -r ${D}/usr/lib/${CSL_TARGET_SYS}
		for link in $(find ${D}/usr/lib -type l); do
			file=$(basename $(readlink $link))
			rm $link
			ln -s ../../lib/$file $link
		done
		ln -s . ${D}/usr/lib/${CSL_TARGET_SYS}
		ln -s . ${D}/lib/${CSL_TARGET_SYS}
		rm ${D}/usr/lib/*.map
